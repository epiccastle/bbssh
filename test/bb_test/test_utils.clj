...
(deftest test-create-file
  (process/sh "rm -f /tmp/bbssh-test-file")
  (utils/create-file (io/file "/tmp/bbssh-test-file") 0644)
  (is (-> (:out (process/sh "ls -l /tmp/bbssh-test-file"))
          (string/starts-with? "-rw-r--r--")))
  (process/sh "rm -f /tmp/bbssh-test-file")
  (utils/create-file (io/file "/tmp/bbssh-test-file") 0755)
  (is (-> (:out (process/sh "ls -l /tmp/bbssh-test-file"))
          (string/starts-with? "-rwxr-xr-x")))
  (process/sh "rm -f /tmp/bbssh-test-file")
  (utils/create-file (io/file "/tmp/bbssh-test-file") 0000)
  (is (-> (:out (process/sh "ls -l /tmp/bbssh-test-file"))
          (string/starts-with? "----------")))
  (process/sh "rm -f /tmp/bbssh-test-file")
  (utils/create-file (io/file "/tmp/bbssh-test-file") 0777)
  (is (-> (:out (process/sh "ls -l /tmp/bbssh-test-file"))
          (string/starts-with? "-rwxrwxrwx")))
  (is (thrown? java.nio.file.FileAlreadyExistsException
               (utils/create-file
                (io/file "/tmp/bbssh-test-file")
                0777))))
